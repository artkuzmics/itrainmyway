...
function t_form_dict(msg) {
    ...
    dict.success = {
        ...
        LT: "Ačiū! Jūsų duomenys buvo pateikta."
    }, dict.successorder = {
        ...
        LT: "Ačiū! Užsakyti sukurta. Prašau palauk. Mes ketiname mokėjimo ..."
    }, dict.email = {
        EN: "Please put a correct e-mail",
        ...
        LT: "Prašome įdėti teisingą elektroninį paštą"
    }, dict.url = {
        ...
        LT: "Prašome įdėti teisingą telefono numerį"
    }, dict.number = {
        ...
        LT: "Prašome įdėti teisingą skaičių"
    }, dict.date = {
        ...
        LT: "Prašome įdėti teisingą datą"
    }, dict.time = {
        ...
        LT: "Prašome įdėti teisingą laiką (VV:mm),"
    }, dict.name = {
        ...
        LT: "Prašome įdėti teisingą vardą (tik kirilica)"
    }, dict.nameeng = {
        ...
        LT: "Prašome įdėti teisingą vardą (tik lotyniškomis raidėmis)"
    }, dict.string = {
        ...
        LT: "Jūs įdėti neteisingus simbolius. Tik raidės, skaičiai ir skyrybos ženklai yra leidžiami"
    }, dict.req = {
        ...
        LT: "Nė vienas iš srityse yra užpildyta"
    }, dict.chosevalue = {
        ...
        LT: "Prašome pasirinkti iš variantų adresą"
    }, dict.deliveryreq = {
        ...
        LT: "Neįmanoma pateikti užsakymą be pristatymo. Prašome atnaujinti puslapį ir bandykite dar kartą"
    };
    var lang = window.browserLang;
    return void 0 !== dict[msg] ? void 0 !== dict[msg][lang] && "" != dict[msg][lang] ? dict[msg][lang] : dict[msg].EN : ""
}
    ...
    $.mask = {
        ...
        mask: function(mask, settings) {
            ...
            if (!mask && this.length > 0) {
                var fn = (input = $(this[0])).data($.mask.dataName);
                return fn ? fn() : void 0
            }
            return settings = $.extend({
                ...
                function keypressEvent(e) {
                    if (!input.prop("readonly")) {
                        ...
                        if (!(e.ctrlKey || e.altKey || e.metaKey || k < 32) && k && 13 !== k) {
                            if (pos.end - pos.begin != 0 && (clearBuffer(pos.begin, pos.end), shiftL(pos.begin, pos.end - 1)), (p = seekNext(pos.begin - 1)) < len && (c = String.fromCharCode(k), tests[p].test(c))) {
                                var proxy;
                                if (shiftR(p), buffer[p] = c, writeBuffer(), next = seekNext(p), android) setTimeout((function() {
                ...
                input.data($.mask.dataName, (function() {
                    ...
                    input.prop("readonly") || (clearTimeout(caretTimeoutId), focusText = input.val(), pos = checkVal(), caretTimeoutId = setTimeout((function() {
                        input.get(0) === document.activeElement && (writeBuffer(), pos == mask.replace("?", "").length ? input.caret(0, pos) : setTimeout((function() {
                            input.caret(pos)
                        }), 50))
                    }), 10))
                })).on("blur.mask", blurEvent).on("keydown.mask", keydownEvent).on("keypress.mask", keypressEvent).on("input.mask paste.mask", (function() {
                    if (!input.prop("readonly")) {
                        if (0 === input.data("tilda-mask").indexOf("+7")) {
                            var value = input.val().replace(/\D/g, "");
                            0 === value.indexOf("78") && input.val(value.substring(2)), 0 !== value.indexOf("8") && 0 !== value.indexOf("7") || input.val(value.substring(1))
                        }
                        setTimeout((function() {
                            var pos = checkVal(!0);
...
function($) {
    window.scriptSysPayment = {}, window.handlerSysPayment = {}, window.tildaForm = {
        ...
        window.tildaForm.captchaCallback = function(token) {
            ...
            var arError = [],
                isEmptyValue = !0;
            if ($jform.find(".js-tilda-rule").each((function() {
                    var req = $(this).data("tilda-req") || 0,
                        rule = $(this).data("tilda-rule") || "none",
                        regExp, str, domainpart, minlength = $(this).data("tilda-rule-minlength") || 0,
                        maxlength = $(this).data("tilda-rule-maxlength") || 0,
                        error = {},
                        val = $(this).val(),
                        valnospace = "";
                    if (error.obj = $(this), error.type = [], val && val.length > 0) {
                        try {
                            valnospace = val.replace(/[\s\u0000—\u001F\u2000-\u200F\uFEFF\u2028-\u202F\u205F-\u206F]/gi, "")
                        } catch (e) {}
                        val = val.trim()
                    }
                    if (val.length > 0 && (isEmptyValue = !1), minlength && (minlength = parseInt(minlength)), maxlength && (maxlength = parseInt(maxlength)), 1 == req && (0 == val.length && 0 == valnospace.length || ("checkbox" == $(this).attr("type") || "radio" == $(this).attr("type")) && 0 == $(this).closest("form").find('[name="' + $(this).attr("name") + '"]:checked').length)) error.type.push("req");
                    else {
                        switch (rule) {
                            case "email":
                                regExp = /^(?!\.)(?!.*\.\.)[a-zA-Zёа-яЁА-Я0-9_\.\-\+]{0,63}[a-zA-Zёа-яЁА-Я0-9_\-\+]@[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}$/gi, val.length > 0 && !val.match(regExp) && error.type.push("email");
                                break;
                            case "url":
                                regExp = /^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9_\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/gi, val.length > 0 && ((str = (str = (str = val.split("//")) && str.length > 1 ? str[1] : str[0]).split("/")) && str.length > 0 && str[0] > "" ? (str = str[0]).match(regExp) || error.type.push("url") : (str && "" != str[0] || error.type.push("url"), str = ""));
                                break;
                            case "phone":
                                regExp = /^[0-9\(\)\-\+]+$/gi, valnospace.length > 0 && !valnospace.match(regExp) ? error.type.push("phone") : (1 == (str = valnospace.replace(/[^0-9]+/g, "")).indexOf("000") || 1 == str.indexOf("111") || 1 == str.indexOf("222") && "5" != str.substring(0, 1) || 1 == str.indexOf("333") || 1 == str.indexOf("444") || 1 == str.indexOf("555") && "0" != str.substring(0, 1) || 1 == str.indexOf("666") && "0" != str.substring(0, 1) || 1 == str.indexOf("8888") && "4" != str.substring(0, 1)) && error.type.push("phone");
                                break;
                            case "number":
                                regExp = /^[0-9]+$/gi, valnospace.length > 0 && !valnospace.match(regExp) && error.type.push("number");
                                break;
                            case "date":
                                var format = {
                                ...
                                valnospace.length > 0 && !valnospace.match(format[$(this).data("tilda-dateformat")] || /^[0-9]{1,4}[\-\.\/][0-9]{1,2}[\-\.\/][0-9]{1,4}$/gi) && error.type.push("date");
                                break;
                            case "time":
                                regExp = /^[0-9]{2}[:\.][0-9]{2}$/gi, valnospace.length > 0 && !valnospace.match(regExp) && error.type.push("time");
                                break;
                            case "name":
                                ...
                                break;
                            case "nameeng":
                                regExp = /^([A-Za-z\s]{1,}((\-)?[A-Za-z\.\s](\')?){0,})*$/i, val.length > 0 && !val.match(regExp) && error.type.push("nameeng");
                                break;
                            case "namerus":
                                regExp = /^([А-Яа-яЁё\s]{1,}((\-)?[А-Яа-яЁё\.\s](\')?){0,})*$/i, val.length > 0 && !val.match(regExp) && error.type.push("namerus");
                                break;
                            case "string":
                            ...
                            case "chosevalue":
                                ...
                                "true" === $(this).attr("data-option-selected") || error.type.push("chosevalue");
                                break;
                            case "deliveryreq":
                                error.type.push("deliveryreq")
                        }
                        minlength > 0 && val.length > 0 && val.length < minlength && error.type.push("minlength"), maxlength > 0 && val.length > 0 && val.length > maxlength && error.type.push("maxlength")
                    }
                    error.type && error.type.length > 0 && (arError[arError.length] = error)
                })), "y" === $jform.attr("data-formcart")) {
                var minOrderSetted = void 0 !== window.tcart_minorder && window.tcart_minorder > 0,
                ...
                if (minOrderSetted)
                    ...
                    else(error = {}).obj = $({}), error.type = [], error.type.push("minorder"), arError.push(error);
                if (minQuantitySetted)
                    if (window.tcart.total >= window.tcart_mincntorder);
                    else(error = {}).obj = $({}), error.type = [], error.type.push("minquantity"), arError.push(error)
            }
            return isEmptyValue && 0 == arError.length && (arError = [{
                obj: "none",
                type: ["emptyfill"]
            }]), arError
        }, window.tildaForm.hideErrors = function($jform) {
            $jform.find(".js-errorbox-all").hide(), $jform.find(".js-rule-error").hide(), $jform.find(".js-error-rule-all").html(""), $jform.find(".js-successbox").hide(), $jform.find(".js-error-control-box .t-input-error").html(""), $jform.find(".js-error-control-box").removeClass("js-error-control-box"), $jform.removeClass("js-send-form-error"), $jform.removeClass("js-send-form-success");
            var $popupError = $("#tilda-popup-for-error");
            $popupError.length > 0 && $popupError.fadeOut()
        }, window.tildaForm.showErrorInPopup = function($jform, arErrors) {
            if (!arErrors || 0 == arErrors.length) return !1;
            var clsInputBoxSelector = $jform.data("inputbox");
            clsInputBoxSelector || (clsInputBoxSelector = ".blockinput");
            ...
            for (var i = 0; i < arErrors.length; i++)
                if (arErrors[i] && arErrors[i].obj) {
                    if (0 == i && "none" == arErrors[i].obj) {
                        sCommonError = '<p class="t-form__errorbox-item">' + t_form_dict("emptyfill") + "</p>";
                        break
                    }
                    for (isFieldErrorBoxExist = 0, isShowCommonErrors = 1, ($fieldgroup = arErrors[i].obj.closest(clsInputBoxSelector).addClass("js-error-control-box")).find(".t-input-error").length > 0 && (isFieldErrorBoxExist = 1), j = 0; j < arErrors[i].type.length; j++) error = arErrors[i].type[j], sError = "", ($errItem = $jform.find(".js-rule-error-" + error)).length > 0 ? "" == $errItem.text() && t_form_dict(error) > "" ? -1 == sCommonError.indexOf(t_form_dict(error)) && (sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + t_form_dict(error) + "</p>") : (sError = $errItem.eq(0).text(), -1 == sCommonError.indexOf(t_form_dict(error)) && (sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + $errItem.eq(0).text() + "</p>")) : t_form_dict(error) > "" && -1 == sCommonError.indexOf(t_form_dict(error)) && (sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + t_form_dict(error) + "</p>"), isFieldErrorBoxExist && ("" == sError && (t_form_dict(error + "field") > "" ? sError = t_form_dict(error + "field") : t_form_dict(error) > "" && (sError = t_form_dict(error))), sError > "" && ($fieldgroup.find(".t-input-error").html(sError), $fieldgroup.find(".t-input-error").fadeIn()))
                } return sCommonError > "" && ($popupError.find(".t-form__errorbox-text").html(sCommonError), $popupError.css("display", "block").fadeIn(), $popupError.find(".t-form__errorbox-item").fadeIn()), window.errorTimerID && window.clearTimeout(window.errorTimerID), window.errorTimerID = window.setTimeout((function() {
                $("#tilda-popup-for-error").fadeOut(), $jform.find(".t-input-error").html("").fadeOut(), window.errorTimerID = 0
            }), 1e4), $jform.off("focus change", ".js-tilda-rule"), $jform.on("focus change", ".js-tilda-rule", (function() {
            ...
            })), $jform.trigger("tildaform:aftererror"), !0
        }, window.tildaForm.showErrors = function($jform, arErrors) {
            if (!arErrors || 0 == arErrors.length) return !1;
            if ("y" == $jform.data("error-popup")) return tildaForm.showErrorInPopup($jform, arErrors);
            var clsInputBoxSelector = $jform.data("inputbox");
            clsInputBoxSelector || (clsInputBoxSelector = ".blockinput");
            for (var $fieldgroup, isFieldErrorBoxExist, isShowCommonErrors, $errItem, sError = "", i = 0; i < arErrors.length; i++)
                if (arErrors[i] && arErrors[i].obj) {
                    if (0 == i && "none" == arErrors[i].obj) {
                        ($errItem = $jform.find(".js-rule-error-all")).html(t_form_dict("emptyfill")), $errItem.css("display", "block").show();
                        break
                    }
                    for (isFieldErrorBoxExist = 0, isShowCommonErrors = 1, ($fieldgroup = arErrors[i].obj.closest(clsInputBoxSelector).addClass("js-error-control-box")).find(".t-input-error").length > 0 && (isFieldErrorBoxExist = 1), j = 0; j < arErrors[i].type.length; j++) error = arErrors[i].type[j], sError = "", ($errItem = $jform.find(".js-rule-error-" + error)).attr("data-rule-filled") ? $errItem.css("display", "block").show() : $errItem.length > 0 ? ("" == $errItem.text() && t_form_dict(error) > "" ? $errItem.html(t_form_dict(error)) : sError = $errItem.eq(0).text(), $errItem.css("display", "block").show()) : t_form_dict(error) > "" && ($errItem = $jform.find(".js-rule-error-all")) && $errItem.length > 0 && ($errItem.html(t_form_dict(error)), $errItem.css("display", "block").show()), isFieldErrorBoxExist && ("" == sError && (t_form_dict(error + "field") > "" ? sError = t_form_dict(error + "field") : t_form_dict(error) > "" && (sError = t_form_dict(error))), sError > "" && $fieldgroup.find(".t-input-error").html(sError))
                } return $jform.find(".js-errorbox-all").css("display", "block").show(), $jform.trigger("tildaform:aftererror"), !0
        }, checkVerifyTildaCaptcha = function(event) {
            if (-1 == event.origin.indexOf(window.tildaForm.endpoint));
            else {
                if ("closeiframe" == event.data) return $("#tildaformcaptchabox").remove(), void $("#js-tildaspec-captcha").remove();
                var capthakey = event.data,
                    $elem = $("#js-tildaspec-captcha");
                $elem && $elem.length > 0 && ($elem.val(capthakey), $("#tildaformcaptchabox").remove(), $elem.closest("form").trigger("submit"))
            }
        }, window.tildaForm.addTildaCaptcha = function($jform, formskey) {
            var randomkey;
            $("#tildaformcaptchabox").remove(), $("#js-tildaspec-captcha").remove(), $jform.append('<input type="hidden" name="tildaspec-tildacaptcha" id="js-tildaspec-captcha">');
            try {
                randomkey = (new Date).getTime() + "=" + parseInt(8 * Math.random())
            } catch (e) {
                randomkey = "rnd=" + parseInt(8 * Math.random())
            }
            $("body").append('<div id="tildaformcaptchabox" style="z-index:10000000;position:fixed; text-align: center; vertical-align: middle; top: 0px; left:0px; bottom: 0px; right: 0px; background: rgba(255,255,255,0.97);"><iframe id="captchaIframeBox" src="//' + window.tildaForm.endpoint + "/procces/captcha/?tildaspec-formid=" + $jform.attr("id") + "&tildaspec-formskey=" + formskey + "&" + randomkey + '" frameborder="0" width="100%" height="100%"></iframe></div>'), window.removeEventListener("message", checkVerifyTildaCaptcha), window.addEventListener("message", checkVerifyTildaCaptcha)
            ...
            if (currencyCode ? (window.tildaForm.currencyForStat = currencyCode, window.tildaForm.tildapayment.currency = currencyCode) : window.tcart.currency && window.tcart.currency > "" && (window.tildaForm.currencyForStat = window.tcart.currency, window.tildaForm.tildapayment.currency = window.tcart.currency), !window.tcart.delivery && $(".t-radio_delivery:checked").length && $(".t-radio_delivery:checked").data("delivery-price") > 0) try {
            ...
            var dNow, offsetFrom_UTC_to_Local = (new Date).getTimezoneOffset();
            window.tildaForm.tildapayment.timezoneoffset = offsetFrom_UTC_to_Local;
            var arProduct, optionlabel, iProductsCount = 0;
            for (window.tcart.products && window.tcart.products.length > 0 && (iProductsCount = window.tcart.products.length), window.tildaForm.tildapayment.products = [], i = 0; i < iProductsCount; i += 1) {
                for (j in product = window.tcart.products[i], arProduct = {}, optionlabel = "", window.tildaForm.tildapayment.products[i] = {}, product)
                    if ("function" != typeof product[j])
                        if ("options" == j)
                            for (var o in window.tildaForm.tildapayment.products[i][j] = {}, product[j]) void 0 === window.tildaForm.tildapayment.products[i][j][o] && (window.tildaForm.tildapayment.products[i][j][o] = {}), product[j][o].option && (window.tildaForm.tildapayment.products[i][j][o].option = product[j][o].option), product[j][o].price && product[j][o].price > 0 && (window.tildaForm.tildapayment.products[i][j][o].price = product[j][o].price), product[j][o].variant && (window.tildaForm.tildapayment.products[i][j][o].variant = product[j][o].variant), product[j][o].option && product[j][o].variant && (optionlabel > "" && (optionlabel += ", "), optionlabel = optionlabel + product[j][o].option + ":" + product[j][o].variant);
                        else window.tildaForm.tildapayment.products[i][j] = product[j];
                product.sku ? arProduct.id = product.sku : product.uid && (arProduct.id = product.uid), arProduct.name = product.name, product.price ? (arProduct.price = product.price, arProduct.quantity = parseInt(product.amount / product.price)) : product.quantity && product.quantity > 1 ? (arProduct.price = product.amount / product.quantity, arProduct.quantity = product.quantity) : (arProduct.price = product.amount, arProduct.quantity = 1), arProduct.name = product.name, optionlabel > "" && (arProduct.name = arProduct.name + "(" + optionlabel + ")"), product.sku && (arProduct.sku = product.sku), product.uid && (arProduct.uid = product.uid), window.tildaForm.arProductsForStat.push(arProduct)
            }
            window.tcart.delivery && window.tcart.delivery.name && (window.tildaForm.tildapayment.delivery = {
                name: window.tcart.delivery.name
            }, window.tcart.delivery && window.tcart.delivery.price && (pricedelivery = window.tcart.delivery.price, window.tildaForm.tildapayment.delivery.price = window.tcart.delivery.price, window.tcart.prodamount > 0 && void 0 !== window.tcart.delivery.freedl && window.tcart.delivery.freedl > 0 && (window.tildaForm.tildapayment.delivery.freedl = window.tcart.delivery.freedl, window.tcart.prodamount - proddiscount >= window.tcart.delivery.freedl && (pricedelivery = 0)), arProduct = {
                name: window.tcart.delivery.name,
                price: pricedelivery,
                quantity: 1,
                id: "delivery"
            ...
            try {
                ...
                ["city", "street", "pickup-name", "pickup-address", "pickup-id", "house", "entrance", "floor", "aptoffice", "phone", "entrancecode", "comment", "service-id", "hash", "postalcode", "country", "userinitials", "onelineaddress"].forEach((function(keyForTildapayment) {
                    window.tcart.delivery && window.tcart.delivery[keyForTildapayment] && (window.tildaForm.tildapayment.delivery[keyForTildapayment] = window.tcart.delivery[keyForTildapayment])
                }))
            } catch (e) {
                console.log(e)
            }
            ...
            if ("y" != $jform.data("formpaymentoff")) {
                if ($jform.find(".js-successbox").length > 0 && ($jform.find(".js-successbox").text() > "" && $jform.data("successmessage", $jform.find(".js-successbox").text()), t_form_dict("successorder") && $jform.find(".js-successbox").html(t_form_dict("successorder")), $jform.find(".js-successbox").show()), $jform.addClass("js-send-form-success"), "link" == arNext.type) return tildaForm.clearTCart($jform), arNext.message && arNext.message > "" && $jform.find(".js-successbox").length > 0 && ($jform.find(".js-successbox").html(arNext.message), $jform.find(".js-successbox").show()), window.location.href = arNext.value, !0;
                if ("form" == arNext.type) {
            ...
            if (!sysname || !src || "http" != src.substring(0, 4)) return console.log("Wrong script parameters."), !1;
            if (window.scriptSysPayment || (window.scriptSysPayment = {}), !window.scriptSysPayment[sysname] || !0 !== window.scriptSysPayment[sysname]) {
                var script = document.createElement("script");
                script.type = "text/javascript", script.src = src, document.body.appendChild(script), window.scriptSysPayment[sysname] = !0
            }
            ...
            var virtPage = "/tilda/" + $jform.attr("id") + "/payment/",
                ...
                currency;
            window.Tilda && "function" == typeof Tilda.sendEventToStatistics && ($("#allrecords").data("tilda-currency") || $(".t706").data("project-currency-code") || $("#allrecords").data("tilda-currency", arArgs.currency), Tilda.sendEventToStatistics(virtPage, virtTitle, virtProduct, virtPrice));
            arArgs.successurl > "" && window.setTimeout((function() {
                window.location.href = arArgs.successurl
            }), 300), $jform.data("successmessage") > "" ? $jform.find(".js-successbox").html($jform.data("successmessage")) : $jform.find(".js-successbox").html(""), $jform.data("successmessage", "");
            var successcallback = $jform.data("success-callback");
            window.tildaForm.successEnd($jform, arArgs.successurl, successcallback), $jform.trigger("tildaform:aftersuccess")
        }, window.tildaForm.stripeLoad = function() {
            if (!0 !== window.stripeapiiscalled) {
                var script = document.createElement("script");
                script.type = "text/javascript", script.src = "https://checkout.stripe.com/checkout.js", document.body.appendChild(script), window.stripeapiiscalled = !0
            }
            ...
            var companyname = arArgs.companyname,
                companylogo = arArgs.companylogo;
            if (companyname || (companyname = window.location.host), !window.stripehandler) {
                ...
                arArgs.zipCode && 1 == arArgs.zipCode && (arStripeInit.zipCode = !0), arArgs.billingAddress && 1 == arArgs.billingAddress && (arStripeInit.billingAddress = !0), arArgs.shipping && 1 == arArgs.shipping && (arStripeInit.shippingAddress = !0), window.stripehandler = window.StripeCheckout.configure(arStripeInit), $(window).on("popstate", (function() {
            ...
            var multiple = 100;
            try {
                arArgs.multiple && arArgs.multiple > 0 && (multiple = parseInt(arArgs.multiple))
            } catch (e) {}
            window.stripehandler.open({
                ...
                shippingAddress: "1" == arArgs.shipping,
            ...
            if (!0 !== window.cloudpaymentsapiiscalled) {
                var script = document.createElement("script");
                script.type = "text/javascript", script.src = "https://widget.cloudpayments.ru/bundles/cloudpayments", document.body.appendChild(script), window.cloudpaymentsapiiscalled = !0
            }
            ...
            var currency = arArgs.currency,
                language = arArgs.language,
                initCP = {};
            if (language || (language = "RUB" == currency || "BYR" == currency || "BYN" == currency || "RUR" == currency ? "ru-RU" : "UAH" == currency ? "uk" : "en-US"), !window.cloudpaymentshandler) {
                if ("object" != typeof window.cp) return window.setTimeout((function() {
                    window.tildaForm.cloudpaymentPay($jform, arArgs)
                }), 200), !1;
                initCP = {
                    language: language
                }, arArgs.applePaySupport && "off" == arArgs.applePaySupport && (initCP.applePaySupport = !1), arArgs.googlePaySupport && "off" == arArgs.googlePaySupport && (initCP.googlePaySupport = !1), window.cloudpaymentshandler = new cp.CloudPayments(initCP)
            }
            var arData = {};
            arData.projectid = arArgs.projectid, arArgs.cloudPayments && (arArgs.cloudPayments.recurrent || arArgs.cloudPayments.customerReceipt) && (arData.cloudPayments = arArgs.cloudPayments);
            var $jpopup = $jform.closest(".t-popup_show");
            return $jpopup && 0 != $jpopup.length || ($jpopup = $jform.closest(".t706__cartwin_showed")), $jpopup.data("old-style", $jpopup.attr("style")), $jpopup.attr("style", "z-index:100"), window.tildaForm.orderIdForStat = arArgs.invoiceId, arArgs.skin || (arArgs.skin = "classic"), window.cloudpaymentshandler.charge({
                ...
                invoiceId: arArgs.invoiceId,
                requireEmail: 1 == arArgs.requireEmail,
                email: arArgs.email,
                skin: arArgs.skin,
                data: arData
            }, (function(options) {
                window.cloudpaymentshandler = !1, $jpopup.attr("style", $jpopup.data("old-style"));
                var virtPage = "/tilda/" + $jform.attr("id") + "/payment/",
                    virtTitle = "Pay order in form " + $jform.attr("id"),
                    virtPrice = arArgs.amount,
                    virtProduct = arArgs.description;
                $("#allrecords").data("tilda-currency", currency), window.Tilda && "function" == typeof Tilda.sendEventToStatistics && Tilda.sendEventToStatistics(virtPage, virtTitle, virtProduct, virtPrice), window.tildaForm.clearTCart($jform), arArgs.successurl > "" && window.setTimeout((function() {
                    window.location.href = arArgs.successurl
                }), 300), $jform.data("successmessage") > "" ? $jform.find(".js-successbox").html($jform.data("successmessage")) : $jform.find(".js-successbox").html(""), $jform.data("successmessage", "");
                var successcallback = $jform.data("success-callback");
                window.tildaForm.successEnd($jform, arArgs.successurl, successcallback), $jform.trigger("tildaform:aftersuccess")
            }), (function(reason, options) {
                if ($jpopup.attr("style", $jpopup.data("old-style")), $jform.find(".js-successbox").hide(), $jform.data("successmessage") > "" ? $jform.find(".js-successbox").html($jform.data("successmessage")) : $jform.find(".js-successbox").html(""), $jform.data("successmessage", ""), window.cloudpaymentshandler = !1, arArgs.failureurl > "") window.location.href = arArgs.failureurl;
                else {
                    $jpopup.find(".t706__cartwin-products").show(), $jpopup.find(".t706__cartwin-prodamount-wrap").show(), $jpopup.find(".t706__form-bottom-text").show(), $jform.find(".t-form__inputsbox").show();
                    try {
                        tcart__lockScroll()
                    } catch (e) {}
                }
            ...
            if (arArgs && "fast" == arArgs.condition) window.tildaForm.sendStatAndShowMessage($jform, arArgs, !0);
                ...
                var $jbankform = $("#formbanktransfer"),
                    arErrors;
                $jbankform.length > 0 && ($jbankform.off("submit"), $jbankform.find(".t-submit").off("click"), $jbankform.find(".t-submit").off("dblclick"), $jbankform.submit((function(e) {
                    if (e.preventDefault(), (arErrors = window.tildaForm.validate($jbankform)) && arErrors.length > 0) return window.tildaForm.showErrors($jbankform, arErrors), !1;
                    $.ajax({
                        type: "POST",
                        ...
                        success: function(json) {
                            if ($("body").removeClass("t-body_popupshowed"), $jbankform.closest(".t-banktransfer").remove(), json || (json = {
                                    error: "Unknown error. Please reload page and try again later."
                                }), json && json.error) return alert(json.error), !1;
                            window.tildaForm.sendStatAndShowMessage($jform, arArgs, !0)
                        },
                        error: function(data) {
                            $("body").removeClass("t-body_popupshowed"), $jbankform.remove(), alert(data)
                        },
                        timeout: 15e3
                    })
                })))
            } else window.tildaForm.sendStatAndShowMessage($jform, arArgs, !0)
        }, window.tildaForm.closeSuccessPopup = function() {
            var $popup = $("#tildaformsuccesspopup");
            $popup.length > 0 && ($("body").removeClass("t-body_success-popup-showed"), /iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream && window.tildaForm.unlockBodyScroll(), $popup.fadeOut("fast"))
            ...
            }))), $("#tildaformsuccesspopuptext").html(message), $popup.fadeIn("fast"), $("body").addClass("t-body_success-popup-showed"), /iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream && setTimeout((function() {
                window.tildaForm.lockBodyScroll()
            }), 500)
        }, window.tildaForm.successEnd = function($jform, successurl, successcallback) {
            $jform.find(".js-successbox").length > 0 && ("" == $jform.find(".js-successbox").text() && t_form_dict("success") && $jform.find(".js-successbox").html(t_form_dict("success")), "y" == $jform.data("success-popup") ? window.tildaForm.showSuccessPopup($jform.find(".js-successbox").html()) : $jform.find(".js-successbox").show()), $jform.addClass("js-send-form-success"), successcallback && successcallback.length > 0 ? eval(successcallback + "($jform)") : successurl && successurl.length > 0 && setTimeout((function() {
        ...
        /* window.tildaForm.send = function($jform, btnformsubmit, formtype, formskey) {
            if (window.tildaForm.tildapayment = !1, ("y" == $jform.data("formcart") || $jform.closest(".t706__orderform").length > 0) && window.tildaForm.addPaymentInfoToForm($jform), 2 == formtype || !formtype && formskey > "") {
                ...
                var d = {};
                if (d = $jform.serializeArray(), d = d.filter((function(object) {
                ...
                return $.ajax({
                    ...
                    success: function(json) {
                        ...
                        if (btnformsubmit.removeClass("t-btn_sending"), btnformsubmit.data("form-sending-status", "0"), btnformsubmit.data("submitform", ""), json && json.error) {
                            ...
                            try {
                                $jform.trigger("tildaform:aftersuccess"), formsendedcallback && formsendedcallback.length > 0 && eval(formsendedcallback + "($jform);")
                            } catch (e) {
                                console.log(e)
                            }
                    ...
                    error: function(error) {
                        ...
                        $errBox && 0 != $errBox.length || ($jform.prepend('<div class="js-errorbox-all"></div>'), $errBox = $jform.find(".js-errorbox-all"));
                        var $allError = $errBox.find(".js-rule-error-all");
                        if ($allError && 0 != $allError.length || ($errBox.append('<p class="js-rule-error-all"></p>'), $allError = $errBox.find(".js-rule-error-all")), !error || 0 == error.status && ts_delta < 100) $allError.html("Request error (opening block content panel). Please check internet connection...");
                        else {
                            if (error && (error.status >= 500 || 408 == error.status || 410 == error.status || 429 == error.status || "timeout" == error.statusText) && -1 !== window.tildaForm.endpoint.indexOf("forms.tilda")) return window.tildaForm.endpoint = "forms2.tildacdn.com", window.tildaForm.send($jform, btnformsubmit, formtype, formskey), !1;
                            error && error.responseText > "" ? $allError.html(error.responseText + ". Please, try again later.") : error && error.statusText ? $allError.html("Error [" + error.statusText + "]. Please, try again later.") : $allError.html("Unknown error. Please, try again later.")
                        }
                        $allError.show(), $errBox.show(), $jform.addClass("js-send-form-error"), $jform.trigger("tildaform:aftererror")
                    },
                    timeout: 15e3
                }), !1
            }
                ...
                return d = $jform.serializeArray(), window.tildaForm.tildapayment && window.tildaForm.tildapayment.amount && d.push({
                    ...
                    success: function(html) {
                        ...
                        if (btnformsubmit.removeClass("t-btn_sending"), btnformsubmit.data("form-sending-status", "0"), btnformsubmit.data("submitform", ""), html && html.length > 0)
                            if ("{" == html.substring(0, 1)) {
                                if ((json = window.JSON && window.JSON.parse ? window.JSON.parse(html) : $.parseJSON(html)) && json.message) "OK" != json.message && $jform.find(".js-successbox").html(json.message);
                                    ...
                                    $errBox && 0 != $errBox.length || ($jform.prepend('<div class="js-errorbox-all"></div>'), $errBox = $jform.find(".js-errorbox-all"));
                                    var $allError = $errBox.find(".js-rule-error-all");
                                    return $allError && 0 != $allError.length || ($errBox.append('<p class="js-rule-error-all">Unknown error. Please, try again later.</p>'), $allError = $errBox.find(".js-rule-error-all")), $allError.html(json.error), $allError.show(), $errBox.show(), $jform.addClass("js-send-form-error"), $jform.trigger("tildaform:aftererror"), !1
                                }
                        ...
                        window.Tilda && "function" == typeof Tilda.sendEventToStatistics ? window.Tilda.sendEventToStatistics(virtPage, virtTitle, "", 0) : ("undefined" != typeof ga && "tilda" != window.mainTracker && ga("send", {
                            ...
                            title: virtTitle,
                            referer: window.location.href
                        })), $jform.trigger("tildaform:aftersuccess"), window.tildaForm.successEnd($jform, successurl, successcallback)
                    },
                    error: function(error) {
                        btnformsubmit.removeClass("t-btn_sending"), btnformsubmit.data("form-sending-status", "0"), btnformsubmit.data("submitform", "");
                        var $errBox = $jform.find(".js-errorbox-all");
        ...
        $(".js-tilda-captcha").each((function() {
            if ($(this).attr("data-tilda-captchakey") > "") {
                ...
                0 == $("#" + idform + "recaptcha").length && $(this).append('<div id="' + idform + 'recaptcha" class="g-recaptcha" data-sitekey="' + $(this).attr("data-tilda-captchakey") + '" data-callback="window.tildaForm.captchaCallback" data-size="invisible"></div>')
            ...
            $(".js-tilda-mask").each((function() {
                var mask = $(this).data("tilda-mask"),
                    maskplaceholder = $(this).data("tilda-mask-holder");
                mask && !$(this).data("tilda-mask-init") && (maskplaceholder && maskplaceholder > "" ? $(this).mask("" + mask, {
                    placeholder: "" + maskplaceholder
                }) : $(this).mask("" + mask), $(this).data("tilda-mask-init", "1"))
            }))
            ...
            str > "" && ($(this).attr("placeholder", str), $(this).data("placeholder", ""))
        ...
        $jallforms.length > 0 && $jallforms.each((function() {
            ...
            if ($activeForm.find(".g-recaptcha").length > 0 && grecaptcha) {
                ...
                if (void 0 === captchaid || "" === captchaid) {
                    var opts = {
                        size: "invisible",
                        sitekey: $activeForm.data("tilda-captchakey"),
                        callback: window.tildaForm.captchaCallback
                    };
                    captchaid = grecaptcha.render($activeForm.attr("id") + "recaptcha", opts), $activeForm.data("tilda-captcha-clientid", captchaid)
                } else grecaptcha.reset(captchaid);
                return grecaptcha.execute(captchaid), !1
            }
            //return window.tildaForm.send($activeForm, btnformsubmit, formtype, formskey), !1
        }));
